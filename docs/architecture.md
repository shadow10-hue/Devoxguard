...
- **Anomaly engine state is in-memory by default**, keyed per analyzer-instance (a single Node process) — it is not persisted or shared across horizontally-scaled instances of the host app, and resets on restart. Setting `DevoxGuardConfig.redisUrl` (see `DEVOXGUARD_REDIS_URI` in `.env.example`) moves the EWMA/sequence/origin-shift state into Redis instead, shared across every instance pointed at the same client — this is what makes horizontal scaling and safe multi-process clustering (§8 below) possible. It remains optional and off by default: absent, the analyzers behave exactly as before. Redis errors *during* a request (as opposed to it simply not being configured) fail open — logged, no anomaly signal for that request — consistent with the fail-open policy in §5, never turning a Redis hiccup into a self-inflicted denial of service.
- **The stats/overview anomaly score trend (`AnomalyScoreTrendTracker`) is still in-memory only**, a rolling buffer capped at 500 samples per process — the Redis option above doesn't cover it yet.
...
- **The DSL grammar stays deliberately small even with boolean composition.** Conditions can combine comparisons/exists/in checks with `and`/`or`, prefix `not`, and parentheses (max 16 levels of nesting; `or` binds loosest, then `and`, then `not`; see `docs/dsl-spec.md` §2). Remaining limits: the right-hand side of `in`/`not in` is a context path only (no literal array syntax like `["admin", "root"]`), and a condition referencing `response.*` anywhere in its tree makes the whole rule response-phase — so mixed request/response conditions can only `journaliser`, never `bloquer`.
- **The internal dashboard API's auth is a single static API key**, proportionate to a demo/stage project — not a general-purpose auth system (no per-user permissions, no key rotation). The comparison is constant-time (`crypto.timingSafeEqual` over SHA-256 digests) and repeated failed attempts are throttled by a dedicated `TokenBucket` (see `InternalApiKeyGuard`, formerly AR-7 in `.github/SECURITY.md`) — but it remains one shared secret for every caller.
...
## 8. Multi-process clustering (`packages/api-demo/src/cluster.ts`)

An opt-in alternative entrypoint (`npm run start:prod:cluster`, separate from the default `start:prod`) that forks one worker process per CPU core via Node's `cluster` module, each running the app's normal `bootstrap()` unchanged. The primary process forks workers, respawns any that exit unexpectedly, and forwards `SIGTERM`/`SIGINT` to every worker for graceful shutdown (guarded by a `shuttingDown` flag, so a signal-triggered exit isn't mistaken for a crash and respawned mid-shutdown).

**Depends on §7's Redis-backed anomaly state to be safe.** Each worker is a fully independent process. Without `DEVOXGUARD_REDIS_URI` configured, each worker holds its own separate in-memory EWMA/sequence-scan/origin-shift state — a given user's requests landing on different workers (the OS/cluster module's round-robin scheduling) fragments anomaly detection, a real detection-evasion risk. Rule-based blocking (`RuleRegistry`/`evaluateRules`) is stateless per-request and unaffected regardless. Do not enable `start:prod:cluster` in production before Redis-backed state is configured and verified, unless that degradation is an accepted, documented tradeoff.
