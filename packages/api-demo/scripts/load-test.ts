...
async function main() {
  process.env.DEVOXGUARD_API_KEY =
    process.env.DEVOXGUARD_API_KEY ?? 'dev-api-key';
  ...
  try {
    ...
    const baseline = await runPhase(
      'Phase 1: baseline (throttled ~2 req/sec, owned resource)',
      {
    ...
    const burst = await runPhase(
      'Phase 2: sustained burst (owned ids 10-14, high rate — frequency + sequence signals)',
      {
        url: BASE_URL,
        headers: { authorization: 'Bearer 1' },
        requests: ['10', '11', '12', '13', '14'].map((id) => ({
          method: 'GET',
          path: `/orders/${id}`,
        })),
    ...
    const idor = await runPhase(
      'Phase 3: sustained IDOR attempts (unowned resource)',
      {
        url: `${BASE_URL}/orders/1`,
    ...
    console.log(
      `Burst requests completed: ${burst.requests.total}, 429s: ${rateLimited}`,
    );
